...
IMPLEMENT_DYNAMIC(TAZZA, CDialogEx)

TAZZA::TAZZA(CWnd* pParent /*=nullptr*/)
...
void TAZZA::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(TAZZA, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &TAZZA::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void TAZZA::OnBnClickedButton1()
{
	...
	CFile file3;
	file3.Open(_T("TAZZA.txt"), CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite);
	file3.SeekToEnd();
	...
	for (int i = 0; i < 55; i++) {
		if (m_seat[i].GetCheck()) {
			if (i <= 10) {
				...
				seat += " ";
				str += seat;
				file3.Write(seatnum, seatnum.GetLength() * sizeof(char) * 2);
			}
			else if (i > 10 && i <= 21) {
...
BOOL TAZZA::OnInitDialog()
{
	...
	int seatnum;
	CFile file3;
	file3.Open(_T("TAZZA.txt"), CFile::modeCreate | CFile::modeNoTruncate | CFile::modeRead);
	CArchive seat(&file3, CArchive::load);
