...
IMPLEMENT_DYNAMIC(BLACKWIDOW, CDialogEx)

BLACKWIDOW::BLACKWIDOW(CWnd* pParent /*=nullptr*/)
...
void BLACKWIDOW::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(BLACKWIDOW, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &BLACKWIDOW::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void BLACKWIDOW::OnBnClickedButton1()
{
	...
	CFile file1;
	file1.Open(_T("BLACKWIDOW.txt"), CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite);
	file1.SeekToEnd();
	...
	for (int i = 0; i < 55; i++) {
		if (m_seat[i].GetCheck()) {
			if (i <= 10) {
				...
				seat += " ";
				str += seat;
				file1.Write(seatnum, seatnum.GetLength() * sizeof(char) * 2);
			}
			else if (i > 10 && i <= 21) {
...
BOOL BLACKWIDOW::OnInitDialog()
{
	...
	int seatnum;
	CFile file1;
	file1.Open(_T("BLACKWIDOW.txt"), CFile::modeCreate | CFile::modeNoTruncate | CFile::modeRead);
	CArchive seat(&file1, CArchive::load);
